...
void SpaceNavJoystick::update() {
    ...
    //  we're only interested in the latest motion event, but we want to take note of any button event that happens
    spnav_event sev;
    while (spnav_poll_event(&sev)) {
        if (sev.type == SPNAV_EVENT_MOTION) {
            ...
            //  the y axis corresponds to pulling the knob up and down, which we use to control the dribbler
            //  moving the knob in other directions unintentionally moves the y axis, so we give it a large deadzone
        ...
        //  we can't control kick speed with the SpaceNavigator mouse, so default to half power
...
void SpaceNavJoystick::open() {
    ...
    _daemonTried = true;
    if (!_daemonConnected) {
        cerr << "Unable to connect to spacenav daemon.  Make sure spacenavd is running if you want to use a 3d mouse to drive" << endl;
    } else {
        cout << "Connected to spacenav daemon!  If a 3d mouse is connected, you can use it to drive" << endl;
    }
}
